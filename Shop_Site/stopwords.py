...
es_stopwords = ['de', 'la', 'que', 'el', 'en', 'y', 'a', 'los', 'del', 'se', 'las', 'por', 'un', 'para', 'con', 'no',
                ...
                'sintiendo', 'sentido', 'sentida', 'sentidos', 'sentidas', 'siente', 'sentid', 'tengo', 'tienes',
...
def word_tokenize(text):
    return text.encode('utf-8').decode().lower().split()


def save_description(principio_activo, accion_terapeutica, concentracion, presentacion, form, language):
    if language == 'es':
        return 'principio activo: ' + principio_activo.encode(
            'utf-8') + ' \n' + 'acción terapéutica: ' + accion_terapeutica.encode(
            'utf-8') + ' \n' + 'concentración: ' + concentracion.encode(
            'utf-8') + ' \n' + 'presentación: ' + presentacion.encode('utf-8') + ' \n' + 'Método: ' + form.encode(
            'utf-8') + ' \n'
    return 'Active Ingredient: ' + principio_activo.encode(
        'utf-8') + ' \n' + 'therapeutic action: ' + accion_terapeutica.encode(
        'utf-8') + ' \n' + 'concentration: ' + concentracion.encode(
        'utf-8') + ' \n' + 'presentation: ' + presentacion.encode('utf-8') + ' \n' + 'Method: ' + form.encode(
        'utf-8') + ' \n'


def remove_stopwords(collection):
    res = []
    for x in collection:
        if x not in es_stopwords:
            res.append(x)
    return res
